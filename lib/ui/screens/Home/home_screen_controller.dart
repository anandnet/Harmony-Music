...
class HomeScreenController extends GetxController {
  ...
  Future<void> loadContent() async {
    ...
    if (isCachedHomeScreenDataEnabled) {
      ...
      if (loaded) {
        final currTimeSecsDiff = DateTime.now().millisecondsSinceEpoch - box.get('homeScreenDataTime');
  ...
  Future<void> loadContentFromNetwork({bool silent = false}) async {
    ...
    try {
      ...
      final homeContentListMap =
          await _musicServices.getHome(limit: Get.find<SettingsScreenController>().noOfHomeScreenContent.value);
      if (contentType == 'TR') {
        final index = homeContentListMap.indexWhere((element) => element['title'] == 'Trending');
        if (index != -1 && index != 0) {
        ...
        final index = homeContentListMap.indexWhere((element) => element['title'] == 'Top music videos');
        if (index != -1 && index != 0) {
          final con = homeContentListMap.removeAt(index);
          quickPicks.value = QuickPicks(List<MediaItem>.from(con['contents']), title: con['title']);
        } else if (index == -1) {
          List charts = await _musicServices.getCharts();
          quickPicks.value = QuickPicks(List<MediaItem>.from(charts[0]['contents']), title: charts[0]['title']);
      ...
      if (quickPicks.value.songList.isEmpty) {
        final index = homeContentListMap.indexWhere((element) => element['title'] == 'Quick picks');
        final con = homeContentListMap.removeAt(index);
        quickPicks.value = QuickPicks(List<MediaItem>.from(con['contents']), title: 'Quick picks');
      }
      ...
      await Hive.box('AppPrefs').put('homeScreenDataTime', DateTime.now().millisecondsSinceEpoch);
  ...
  Future<void> changeDiscoverContent(dynamic val, {String? songId}) async {
    ...
    if (val == 'QP') {
      ...
      quickPicks_ =
          QuickPicks(List<MediaItem>.from(homeContentListMap[0]['contents']), title: homeContentListMap[0]['title']);
      ...
      try {
        ...
        final index = val == 'TMV'
            ...
            : charts.length == 4
                ? 3
                : 2;
        quickPicks_ = QuickPicks(List<MediaItem>.from(charts[index]['contents']), title: charts[index]['title']);
      } catch (e) {
        printERROR("Seems ${val == "TMV" ? "Top music videos" : "Trending songs"} currently not available!");
      }
    } else {
  ...
  Future<void> cachedHomeScreenData({
    ...
    if (updateQuickPicksNMiddleContent) {
      await homeScreenData.putAll({
        ...
        'quickPicks': _getContentDataInJson(quickPicks.value.songList, isQuickPicks: true),
  ...
  List<Map<String, dynamic>> _getContentDataInJson(List content, {bool isQuickPicks = false}) {
    if (isQuickPicks) {

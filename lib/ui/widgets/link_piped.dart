...
class LinkPiped extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return CommonDialog(
        child: Obx(() => Container(
              height: pipedLinkedController.selectedInst.value == "custom" ? 400 : 365,
              ...
              child: Column(
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Obx(() => DropdownButton(
                        ...
                        items: pipedLinkedController.pipedInstList
                            .map(
                              (element) => DropdownMenuItem(value: element.apiUrl, child: Text(element.name)),
                            )
                            .toList(),
                        onChanged: (val) {
                  ...
                  Obx(() => pipedLinkedController.selectedInst.value == "custom"
                      ? ModifiedTextField(
                          controller: pipedLinkedController.instApiUrlInputController,
                  ...
                  Obx(() => ModifiedTextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          suffixIcon: IconButton(
                            ...
                            onPressed: () => pipedLinkedController.passwordVisible.value =
...
class PipedLinkedController extends GetxController {
  ...
  final pipedInstList = <PipedInstance>[PipedInstance(name: "selectAuthIns".tr, apiUrl: "")].obs;
  ...
  Future<void> getAllInstList() async {
    _pipedServices.getAllInstanceList().then((res) {
      if (res.code == 1) {
        pipedInstList
            .addAll(List<PipedInstance>.from(res.response) + [PipedInstance(name: "customIns".tr, apiUrl: "custom")]);
      } else {
        errorText.value = "${res.errorMessage ?? "errorOccuredAlert".tr}! ${"customInsSelectMsg".tr}";
        pipedInstList.add(PipedInstance(name: "customIns".tr, apiUrl: "custom"));
      }
    });
  }
  ...
  void link() {
    ...
    if (userName.isEmpty ||
        ...
        (instApiUrlInputController.hasListeners && instApiUrlInputController.text.isEmpty)) {
      errorText.value = "allFieldsReqMsg".tr;
      return;
    }
    _pipedServices
        .login(selectedInst.toString() == 'custom' ? instApiUrlInputController.text : selectedInst.toString(), userName,
            password)
        .then((res) {
      if (res.code == 1) {
